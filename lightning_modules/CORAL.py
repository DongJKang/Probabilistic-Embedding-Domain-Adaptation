...
from utils.losses import KLDLoss, CORALLoss
from models import build_network
import numpy as np
from torchmetrics.classification import F1Score

class CORAL(L.LightningModule):
    def __init__(self, config):
        ...
        self.domain_criterion = CORALLoss()
    ...
    def training_step(self, batch, batch_idx):
        ...
        output = self.classifier(src_feat).squeeze()

        class_loss = self.class_criterion(output, src_class_label.float() if self.binary else src_class_label.long())
        domain_loss = self.domain_criterion(src_feat, tgt_feat)
